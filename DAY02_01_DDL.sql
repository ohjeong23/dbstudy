...
/*
테이블 수정하기
1.칼럼 추가하기     : ALTER TABLE 테이블 명 ADD 칼럼명 데이터 타입 제약조건 
2.칼럼 수정하기     : ALTER TABLE 테이블 명 MODIFY 칼럼명 데이터 타입 제약조건
3.기존 칼럼 삭제하기 : ALTER TABLE 테이블 명 DROP COLUMN 칼럼명
4.칼럼 이름 바꾸기   : ALTER TABLE 테이블 명 RENAME CLUMN 기존 칼럼명 TO 신규 칼럼명
5.테이블 이름 바꾸기 : ALTER TABLE 테이블 명 RENAME TO 신규 테이블 명
*/
--1.BANK_T에 은행 연락처(BANK_TEL) 칼럼을 추가하시오.
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(30 BYTE);
--2.BANK_T의 은행명(BANK_NAME) 칼럼을 VARCHAR2(15 BYTE)로 바꾸고 필수 제약 조건을 지정
...
--5.CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오.'VIP','GOLD','SILVER','BRONZE' 
--중 하나의 값을 가지도록 설정하시오.
...
ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR(30 BYTE) NULL ;
--반드시 NULL 해야함 안하면 바뀌는게 없음
...
/*
 테이블 수정하기 - PK/FK 제약조건
 1. PK
   1)PK 추가 : ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건명 PRINMARY KEY(지정할 칼럼)
   2)PK 삭제 : 
       (1) ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건명
       (2) ALTER TABLE 테이블명 DROP PRIMARY KEY
 2. FK
   1) 추가 : ALTER TABLE 자식 테이블명 ADD CONSTRAINT 제약조건명 FOREIGN KEY(칼럼명) 
   REFERENCES 부모테이블명(참조할 칼럼)
   2) 삭제 : ALTER TALBE 테이블명 DROP CONSTRAINT 제약조건명
   3) 비활성화 : ALTER TABLE 테이블명 DISABLE CONSTRAINT 제약조건명
   4) 활성화 : ALTER TALBE 테이블명 ENABLE CONSTRAINT 제약조건명
*/
 CONSTRAINT FK_CUSTOMER_BANK FOREIGN KEY (BANK_CODE) REFERENCES BANK_T(BANK_CODE)
