...
/*
   ...
   2.테이블이 필요 없는 조회(DQL)의 경우에도 FROM절까지 작성해야 하는데 
    이 때 사용한다.
   3.1개의 칼럼과 1개의 행(ROW)로 구성되어 있다.
   4.DUMMY 칼럼 
     X     값
*/
/*
  1. 문자 -> 숫자 변환
    TO_NUMBER(문자 [예'12345'])
*/
...
, TO_CHAR(SALARY,'999,999') -- 6자리 문자열로 변환, 천 단위 구분 기호 사용, 
-- 원본이 7자리 이므로 정상 변환이 안 됨,자리수가 모자라서 오류가 남,안보임 ###
FROM EMPLOYEE_T;
...
,TO_DATE(HIRE_DATE,'HH/MI/SS') -- 입력된 값을 시/분/초 (오류 발생)
FROM EMPLOYEE_T;
